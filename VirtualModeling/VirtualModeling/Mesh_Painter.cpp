...
void Mesh_Painter::draw_meshes(float rad, float yaw, float pitch, float scale, float fovy, float aspect, float zNear, float zFar)
{
	for (unsigned int i = 0; i < this->m_my_meshes_.size(); i++)
	{
		...
		float isShadow = 0.0;
		GLuint modelViewMatrixID = glGetUniformLocation(this->program_all[i], "modelViewMatrix");
		GLuint projMatrixID = glGetUniformLocation(this->program_all[i], "projMatrix");
		GLuint rotateMatrixID = glGetUniformLocation(this->program_all[i], "rotateMatrix");

		float x1 = rad * cos(pitch * DegreesToRadians) * cos(yaw * DegreesToRadians);
		float z1 = rad * cos(pitch * DegreesToRadians) * sin(yaw * DegreesToRadians);
		float y1 = rad * sin(pitch * DegreesToRadians);

		CameraScene::modelMatrix = mat4(1.0);
		CameraScene::viewMatrix = CameraScene::lookAt(vec4(x1, y1, z1, 1), vec4(0, 0, 0, 1), vec4(0, 1, 0, 0.0));
		CameraScene::projMatrix = CameraScene::perspective(fovy, aspect, zNear, zFar);

		mat4 modelViewMatrix = CameraScene::viewMatrix * CameraScene::modelMatrix;
		glUniformMatrix4fv(modelViewMatrixID, 1, GL_TRUE, &modelViewMatrix[0][0]);
		glUniformMatrix4fv(projMatrixID, 1, GL_TRUE, &CameraScene::projMatrix[0][0]);
...
void Mesh_Painter::update_vertex_buffer()
{
	...
	for (unsigned int m_i = 0; m_i < this->m_my_meshes_.size(); m_i++)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(vec3)*num_face * 3 + sizeof(vec3)*num_face * 3 + sizeof(vec3)*num_face * 3
		...
		{
			int index = face_list[3 * i];
			points[3 * i] = vec3((normal_list[index * 3 + 0]), (normal_list[index * 3 + 1]), (normal_list[index * 3 + 2]));
			index = face_list[3 * i + 1];
			points[3 * i + 1] = vec3((normal_list[index * 3 + 0]), (normal_list[index * 3 + 1]), (normal_list[index * 3 + 2]));
			index = face_list[3 * i + 2];
			points[3 * i + 2] = vec3((normal_list[index * 3 + 0]), (normal_list[index * 3 + 1]), (normal_list[index * 3 + 2]));
		}
		glBufferSubData(GL_ARRAY_BUFFER, offset, sizeof(vec3) * num_face * 3, points);
		offset += sizeof(vec3) * num_face * 3;
		delete[] points;
		...
		{
			int index = face_list[3 * i];
			points[3 * i] = vec3((color_list[index * 3 + 0]), (color_list[index * 3 + 1]), (color_list[index * 3 + 2]));
			index = face_list[3 * i + 1];
			points[3 * i + 1] = vec3((color_list[index * 3 + 0]), (color_list[index * 3 + 1]), (color_list[index * 3 + 2]));
			index = face_list[3 * i + 2];
			points[3 * i + 2] = vec3((color_list[index * 3 + 0]), (color_list[index * 3 + 1]), (color_list[index * 3 + 2]));
		}
		glBufferSubData(GL_ARRAY_BUFFER, offset, sizeof(vec3) * num_face * 3, points);
		offset += sizeof(vec3) * num_face * 3;
		delete[] points;
