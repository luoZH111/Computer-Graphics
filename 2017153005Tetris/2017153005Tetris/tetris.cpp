...
void newtile()
{
	...
	tilepos = vec2(5 , 19);
	rotation = Random()%4;//随机方向 
	int type = Random()%7;//随机类型 
	tiletype=type;
	for (int i = 0; i < 4; i++)
	{
	...
	// 给新方块赋上颜色
	vec4 newcolours[24];
	vec4 cur_color= Color[Random()%5];//随机颜色 
	tilecolor=cur_color;
	for (int i = 0; i < 24; i++)
...
void init()
{
	...
	for (int i = 0; i < 20; i++)
		for (int j = 0; j < 10; j++)
		{
			vec4 p1 = vec4(33.0 + (j * 33.0), 33.0 + (i * 33.0), .5, 1);//左下 
			vec4 p2 = vec4(33.0 + (j * 33.0), 66.0 + (i * 33.0), .5, 1);//左上 
			vec4 p3 = vec4(66.0 + (j * 33.0), 33.0 + (i * 33.0), .5, 1);//右下 
			vec4 p4 = vec4(66.0 + (j * 33.0), 66.0 + (i * 33.0), .5, 1);//右上 
...
bool checkvalid(vec2 cellpos)
{
	if((cellpos.x >=0) && (cellpos.x < 10) && (cellpos.y >= 0) && (cellpos.y < 20) ){
		if(board[(int)cellpos.x][(int)cellpos.y]==true) return false;//如果该位置被填充，则不能走该步 
		return true;
	}
	else
		return false;
...
//		puts("");
//	}
	//检查是否有满行的情况 st为第一个出现行的位置 ed为最后一个出现满行的位置 
	//出现满行的情况，则对这些行进行消除 同时上面未满行的木块会掉落 
	int st=20,ed=0;
	for(int i=0;i<20;i++){
	...
	if(!gameover)
	{
		switch(key)
		{
			...
			case GLUT_KEY_DOWN: // 向下按键移动方块
				...
				{
					settile();
					newtile();//在方块移动到窗口底部的时候 新的方块出现并重复上述移动过程
				}
				break;
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("俄罗斯方块-2017153005");
	glewInit();
	init();
	puts("start");
